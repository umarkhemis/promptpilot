...
// ── API call (add this to your api.ts too) ────────────────────────────────────
async function sendEmailInvite(workspaceId: string, email: string) {
  const token = (() => {
    try {
      return JSON.parse(localStorage.getItem("promptpilot-store") || "{}").state?.token;
    } catch { return null; }
  })();
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"}/api/workspaces/${workspaceId}/invite/email`,
    {
      method: "POST",
      headers: {
...
function EmailInviteSection({ workspaceId }: { workspaceId: string }) {
  ...
  async function handleSend() {
    ...
    setError(null);
    try {
      await sendEmailInvite(workspaceId, email.trim());
      setSent(true);
      setEmail("");
    ...
    <div className="mb-6">
      ...
      <div className="flex gap-2">
        <div className="relative flex-1">
          <Mail className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5" style={{ color: MUTED }} />
          <input
            value={email}
            ...
            style={{ background: SURFACE, border: `1px solid ${error ? "rgba(239,68,68,0.4)" : BORDER}`, color: TEXT }}
            onFocus={(e) => { e.currentTarget.style.borderColor = `${ACCENT}80`; }}
            onBlur={(e) => { e.currentTarget.style.borderColor = error ? "rgba(239,68,68,0.4)" : BORDER; }}
          />
        </div>
        <button
          onClick={handleSend}
          disabled={loading || !email.trim() || !email.includes("@")}
          className="flex items-center gap-1.5 px-4 py-2.5 rounded-xl text-sm font-semibold text-white transition-opacity"
          style={{
            background: sent ? "rgba(52,211,153,0.2)" : `linear-gradient(135deg, ${ACCENT}, #8B5CF6)`,
            border: sent ? "1px solid rgba(52,211,153,0.3)" : "none",
            color: sent ? "#34d399" : "white",
            opacity: loading || (!email.trim() || !email.includes("@")) ? 0.5 : 1,
          }}>
          {loading
...
function WorkspacePanel({ ws, onClose, onDeleted }: {
  ws: Workspace;
  onClose: () => void;
  onDeleted: () => void;
}) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4"
      ...
      <div className="w-full max-w-lg rounded-2xl p-6 relative max-h-[90vh] overflow-y-auto"
        ...
        {isAdmin && (
          <>
            {/* Email invite */}
            <EmailInviteSection workspaceId={ws.id} />
            ...
            <div className="mb-6">
              ...
              <p className="text-[11px] mt-1.5" style={{ color: MUTED }}>
                Anyone with this link can join. Regenerating invalidates the old link.
              </p>
            </div>
          </>
        )}
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
        ...
        {loading ? (
          ...
          <div className="text-center py-20 rounded-2xl"
            ...
            <p className="text-sm mb-5" style={{ color: MUTED }}>Create one and invite your team.</p>
            ...
            {workspaces.map((ws) => (
              <button key={ws.id}
                ...
                <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0"
                  ...
                  <Users className="w-4 h-4" style={{ color: "#a78bfa" }} strokeWidth={1.75} />
                </div>
                <div className="flex-1 min-w-0">
